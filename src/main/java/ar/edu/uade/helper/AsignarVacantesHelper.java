...
public class AsignarVacantesHelper {
	...
	private void asignarVacantesAEsteCurso(Curso disVsVac, List<Curso> cursos) {
		...
		for (Vacante auxVac : disVsVac.getVacantes()) {
			...
			if (vacantesDisponiblesParaEsteCurso == 0) {
				break;
			}
		}
	...
	private void buscarVacanteParaEsteCurso(Vacante vacante, int id, List<Curso> cursos, int cantFaltantes) {
	...
	private void borrasLasOtrasVacantes(Vacante first, List<Curso> cursos, List<Curso> cursosCompletos) {
		List<Curso> aux = new ArrayList<Curso>();
		aux.addAll(cursos);
		// buscarCursoById(first.getCurso().getId(),
		// cursos).getVacantes().remove(first);
		...
		for (Curso curso : aux) {
			...
			for (Vacante vac : curso.getVacantes()) {
				if (vac.getPreinscripcion().getId() == first.getPreinscripcion().getId() && !vac.isEstaAprobada()) {
					System.out.println("Descartos la vacante " + vac.getId() + " del aspirante con numero de documento "
